...
from glm_critic.sources import FreshRSSSource, HttpSource, SourceError, source_for
...
class ReaderTests(unittest.TestCase):
    def test_fever_form_auth_pagination_and_normalization(self):
        ...
        def open_request(req, timeout):
            ...
            if 'unread_item_ids' in params:
                return Response({'auth': 1, 'unread_item_ids': ','.join(map(str, range(1, 76))),
                                 'feeds': [{'id': 5, 'title': 'Public source'}]})
            ids = params['with_ids'][0].split(',')
            self.assertLessEqual(len(ids), 50)
            return Response({'auth': 1, 'items': [
                {'id': i, 'feed_id': 5, 'title': ' Test ', 'html': '<p>body</p>',
                 'url': f'https://example.org/{i}', 'created_on_time': 1700000000 + int(i),
                 'is_saved': 0, 'is_read': 0} for i in ids]})
    ...
    def test_auth_failure_is_not_empty_success(self):
        source = FreshRSSSource(HttpSource('http://reader', opener=lambda *a, **k: Response({'auth': 0})))
...
class ProviderTests(unittest.TestCase):
    def test_wire_contracts(self):
        replies = {
            'chat': {'choices': [{'message': {'content': '[]'}, 'finish_reason': 'stop'}],
            ...
            'anthropic': {'stop_reason': 'end_turn', 'content': [
                {'type': 'thinking', 'thinking': 'private'}, {'type': 'text', 'text': '[]'}],
                'usage': {'input_tokens': 3, 'output_tokens': 4}},
        }
        for protocol, reply in replies.items():
            with self.subTest(protocol=protocol):
                ...
                text, usage = call_judge('prompt', Settings(judge_api=protocol,
                                        judge_model='test-frontier', judge_key='secret'), request)
                self.assertEqual(text, '[]')
                self.assertEqual(usage, {'prompt_tokens': 3, 'completion_tokens': 4})

    def test_incomplete_output_is_rejected(self):
        for protocol, data in [('chat', {'choices': [{'finish_reason': 'length'}]}),
                               ('responses', {'status': 'incomplete'}),
                               ('anthropic', {'stop_reason': 'max_tokens'})]:
            with self.subTest(protocol=protocol), self.assertRaises(JudgeError):
                call_judge('p', Settings(judge_api=protocol), lambda *a, data=data, **k: Response(data))
    ...
    def test_config_rejects_unknown_protocol_and_unauthenticated_notify(self):
        for env in [{'JUDGE_API': 'unknown'}, {'SOURCE_TYPE': 'unknown'},
                    {'NOTIFY_URL': 'http://orchestrator'}, {'CRITIC_BATCH': '0'}]:
...
class StateTests(unittest.TestCase):
    def test_service_dry_run_does_not_write_or_star(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            source = Mock()
            source.entries.return_value = ([{'id': 1, 'title': 'x', 'url': 'https://example.org'}], 1)
            result = Mock()
            result.as_dict.return_value = {'failures': [], 'items': []}
            result.failures = []
            with patch('glm_critic.service.source_for', return_value=source), patch(
                'glm_critic.service.critique', return_value=result
            ) as judge:
    ...
    def test_provider_and_source_changes_invalidate_cache(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for key, value in [('judge_model', 'another'), ('judge_url', 'http://other'),
                               ('judge_api', 'responses'), ('source_url', 'http://reader')]:
    ...
    def test_retry_usage_counts_both_paid_attempts(self):
        with tempfile.TemporaryDirectory() as directory:
            judge = Mock(side_effect=[('prose', {'prompt_tokens': 10, 'completion_tokens': 20}),
                                     ('[{"i":0,"s":9}]', {'prompt_tokens': 3, 'completion_tokens': 4})])
            r = critique([{'id': 1, 'title': 'A'}], Settings(), Rubric(),
                         VerdictLog(Path(directory) / 'v.jsonl'), judge_call=judge)
            self.assertEqual((r.prompt_tokens, r.completion_tokens), (13, 24))

    def test_notification_auth_replay_and_failed_ack(self):
        with tempfile.TemporaryDirectory() as directory:
            s = Settings(log_path=str(Path(directory) / 'v.jsonl'),
                         notify_url='http://orchestrator', notify_token='secret')
            ...
            with patch('glm_critic.service.notify', return_value=True) as send:
                ...
                self.assertTrue(notify_pending(s, {'items': payload['items'] + [
                    {'notification_key': 'three'}]}))
